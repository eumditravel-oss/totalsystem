...
물량을 하드코딩하지 않고, 도형(템플릿)마다 **편집 가능한 산식**으로 정의하는 엔진 구조입니다.

- **도형(Shape)**: 부재 분류(기초·기둥·보·슬래브·벽체·계단)별로 변수(치수·간격·철근규격)와 물량 산식(콘크리트·거푸집·버림·잡석·철근)을 가짐. 화면에서 산식을 직접 편집 가능.
- **수식 엔진**: `+ − × ÷ ^ ( )`, 변수 치환, 함수(`ROUND/FLOOR/CEIL/MIN/MAX`) 지원.
- **철근 산출**: `본수 × (순길이 + 정착 + 이음 + 후크) × 단위중량`. 간격(@) 기반으로 본수를 산정(`FLOOR(길이/간격)+1`).
- **정착·이음 룩업표**: 콘크리트강도 × 철근규격 × 부재종류 × (상부/일반/압축)으로 정착길이(mm)를 조회, 이음 = 일반정착 × 배율(기본 1.3).
- **분석표**: 부재분류(행) × 강도/규격(열) 매트릭스로 콘크리트·거푸집·철근·버림·잡석을 집계하고, 소계·할증·합계 산출.

기본 내장 도형: 독립기초, 매트기초, 사각기둥, 일반보, 슬래브, 벽체 (모두 산식 편집 가능).
...
src/
  ...
  lib/
    ...
    formula.ts            수식 평가 엔진 (RC 산식)
    rcEngine.ts           RC 도형·정착표·산출·집계 엔진
    rcShapes.ts           기본 내장 도형 + 샘플
